...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent data) {
        int screen = AdvancedLauncher.getScreen();

        if (!installShortcut(context, data, screen)) {
            // The target screen is full, let's try the other screens
            for (int i = 0; i < AdvancedLauncher.SCREEN_COUNT; i++) {
    ...
    private boolean installShortcut(Context context, Intent data, int screen) {
        if (findEmptyCell(context, mCoordinates, screen)) {
            ...
            // different places)
            boolean duplicate = data.getBooleanExtra(AdvancedLauncher.EXTRA_SHORTCUT_DUPLICATE, true);
            if (duplicate || !LauncherModel.shortcutExists(context, name, intent)) {
                AdvancedLauncher.addShortcut(context, data, cell, true);
            }
    ...
    private static boolean findEmptyCell(Context context, int[] xy, int screen) {
        final int xCount = AdvancedLauncher.NUMBER_CELLS_X;
        final int yCount = AdvancedLauncher.NUMBER_CELLS_Y;
