...
public class LauncherModel {
     ...
     */
    synchronized boolean loadApplications(boolean isLaunching, AdvancedLauncher launcher,
    ...
    private synchronized void startApplicationsLoader(AdvancedLauncher launcher, boolean isLaunching) {
    ...
    synchronized void addPackage(AdvancedLauncher launcher, String packageName) {
        if (mApplicationsLoader != null && mApplicationsLoader.isRunning()) {
            startApplicationsLoader(launcher, false);
            return;
        }
    ...
    synchronized void removePackage(AdvancedLauncher launcher, String packageName) {
        if (mApplicationsLoader != null && mApplicationsLoader.isRunning()) {
    ...
    synchronized void updatePackage(AdvancedLauncher launcher, String packageName) {
        if (mApplicationsLoader != null && mApplicationsLoader.isRunning()) {
            startApplicationsLoader(launcher, false);
            return;
        }
    ...
    synchronized void syncPackage(AdvancedLauncher launcher, String packageName) {
        if (mApplicationsLoader != null && mApplicationsLoader.isRunning()) {
            startApplicationsLoader(launcher, false);
            return;
        }
    ...
    private boolean syncLocked(AdvancedLauncher launcher, String packageName) {
        final PackageManager packageManager = launcher.getPackageManager();
        final List<ResolveInfo> matches = findActivitiesForPackage(packageManager, packageName);
    ...
    private boolean addEnabledAndUpdateActivities(List<ResolveInfo> matches,
            ApplicationsAdapter adapter, AdvancedLauncher launcher) {
    ...
    public void openSubMenu(String title) {
        ...
        SubMenuSettings.activeLauncher.getWorkspace().addInScreen(menu, AdvancedLauncher.getScreen(), 0, 0, 4, 4);
    ...
    private class ApplicationsLoader implements Runnable {
        ...
        ApplicationsLoader(AdvancedLauncher launcher, boolean isLaunching) {
     ...
     */
    void loadUserItems(boolean isLaunching, AdvancedLauncher launcher, boolean localeChanged,
    ...
    private class DesktopItemsLoader implements Runnable {
        ...
        DesktopItemsLoader(AdvancedLauncher launcher, boolean localeChanged, boolean loadApplications,
                boolean isLaunching) {
                    ...
                    try {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_WIDGET_SEARCH:
                            ...
                            if (container != LauncherSettings.Favorites.CONTAINER_DESKTOP) {
                                e(AdvancedLauncher.LOG_TAG, "Widget found where container "
                        ...
                        w(AdvancedLauncher.LOG_TAG, "Desktop items loading interrupted:", e);
                    }
                }
            } finally {
                c.close();
    ...
    private static void loadLiveFolderIcon(AdvancedLauncher launcher, Cursor c, int iconTypeIndex,
     ...
     */
    private ApplicationInfo getApplicationInfoShortcut(Cursor c, AdvancedLauncher launcher,
